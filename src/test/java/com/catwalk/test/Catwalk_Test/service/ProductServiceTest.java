...
class ProductServiceTest {
  ...
  void before(){
    ...
    item1 = generator.nextObject(Product.class);
    item2 = generator.nextObject(Product.class);
    service = new ProductService(this.repo);
  }

  @Test
  void shouldSave(){
    Mockito.when(repo.save(Mockito.any(Product.class))).thenReturn(item1);
    Product data = service.save(Product.builder().build());
  ...
  void findAll(){
    ...
    List<Product> all = StreamSupport.stream(service.findAll().spliterator(), false).collect(Collectors.toList());
  ...
  void search(){
    ...
    Page<Product> page = new PageImpl<>(Arrays.asList(item1,item2));
    Mockito.when(repo.findAll(pageRequest)).thenReturn(page);
    List<Product> all = new ArrayList<>(service.search(null, pageRequest).getContent());
    Assertions.assertEquals(2, all.size());
  }
